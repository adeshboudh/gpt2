...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # calculate query, key, value for all heads in batch and move head forward to be the 2nd dimension
        ...
        # y = self.resid_dropout(y) # (B, T, C), output projection
        y = self.c_proj(y)
        return y
...
@dataclass
class GPTConfig:
    block_size: int = 1024 # maximum number of tokens in the input sequence
    vocab_size: int = 50257 # number of tokens in the vocabulary + 256 bytes tokens + 1 <|endoftext|> token
    n_layer: int = 12 # number of transformer layers
    n_head: int = 12 # number of attention heads
    n_embd: int = 768 # dimension of the token embeddings
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        # initialize minGPT model from scratch
        ...
        # the openai checkpoint use a "Conv1D" module, but we only want to use a vanilla Linear layer
        # this means that we need to transpose the weights for the attention and projection layers
        assert len(sd_keys_hf) == len(sd_keys), f"mismatch in number of keys: {len(sd_keys_hf)} != {len(sd_keys)}"
        for k in sd_keys_hf:
            if any(k.endswith(w) for w in transposed):
                # transpose the weights for the attention and projection layers
                assert sd_hf[k].shape[::-1] == sd[k].shape
                with torch.no_grad():
                    sd[k].copy_(sd_hf[k].T)
            else:
                # vanills copy over the other parameters
...
# torchrun command sets the env variabels RANK, WORLD_SIZE, LOCAL_RANK
ddp = int(os.environ.get('RANK', -1)) != -1 # if we are not in ddp, use single gpu
if ddp:
    # use of DDP atm demands CUDA, we set the device appropriately according to rank
    assert torch.cuda.is_available(), "for now i think we need CUDA to use DDP"
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=device_type, dtype=torch.bfloat16):
            logits, loss = model(x, y)
        # we have to scale the loss by grad_accum_steps
    ...
    if master_process:
        print(f"step {step:5d} | loss: {loss_accum.item():.6f} | lr {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_second:.2f}")
        with open(log_file, "a") as f:
